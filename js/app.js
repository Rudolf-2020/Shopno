...
let headerBase = document.querySelector('.header__base');
...
if (iconMenu) {
	iconMenu.addEventListener("click", function () {
		if (!iconMenu.classList.contains("active")) {
			...
			headerBase.classList.add("active");
			...
			headerBase.classList.remove("active");
			if (lockPadding.length > 0) {
				for (let index = 0; index < lockPadding.length; index++) {
					const el = lockPadding[index];
...
$('.goto').click(function () {
	...
	if ($('.menu__body').hasClass('active')) {
		$('.header__base,.menu__body,.icon-menu').removeClass('active');
		$('.lock-padding').css({ paddingRight: 0 });
		$('body').removeClass('lock');
		$('body').css({ paddingRight: 0 });
	}
	return false;
...
function fsblock() {
	...
	$('.start-page').css('min-height', h);
}
fsblock();


let serviColumn = document.querySelectorAll('.services__column');
let servInfoLink = document.querySelectorAll('.serv-info__link');
let servTextBody = document.querySelectorAll('.serv-text__body');
let servTextClose = document.querySelectorAll('.serv-text__close');

for (let i = 0; i < serviColumn.length; i++) {
	let servicColumnInd = serviColumn[i];
	servicColumnInd.addEventListener("click", function (e) {
		if (e.target === servInfoLink[i]) {
			servTextBody[i].classList.add('active');
		}
	})
}

for (let i = 0; i < servTextClose.length; i++) {
	let servTextCloseInd = servTextClose[i];
	servTextCloseInd.addEventListener("click", function () {
		servTextBody[i].classList.remove('active');
	})
}
